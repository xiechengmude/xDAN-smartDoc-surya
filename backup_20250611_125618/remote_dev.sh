...
start_service() {
    ...
    # 启动新的tmux会话（使用uv环境）
    ssh $REMOTE_HOST "cd $DEFAULT_REMOTE_PATH && tmux new-session -d -s surya-api 'source ~/.local/bin/env && uv run python main.py --port $DEFAULT_PORT --host 0.0.0.0 --max-concurrent $DEFAULT_MAX_CONCURRENT'"
