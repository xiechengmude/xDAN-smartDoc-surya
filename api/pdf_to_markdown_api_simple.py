...
def load_surya_modules():
    ...
    try:
        ...
        return load_predictors, TaskNames, settings
...
class BatchConversionRequest(BaseModel):
    ...
    max_pages: Optional[int] = Field(None, description="每个PDF的最大处理页数")
    enable_math: bool = Field(True, description="是否启用数学公式识别")
    task_name: str = Field("ocr_with_boxes", description="OCR任务类型")
...
async def process_pdf_optimized(
    ...
    async with task_semaphore:
        ...
        try:
            ...
            for i in range(0, pages_to_process, effective_batch_size):
                batch_end = min(i + effective_batch_size, pages_to_process)
                batch_pages = list(range(i, batch_end))
                batch_images = []
                ...
                # 更新处理进度
                tasks[task_id]["processed_pages"] = batch_end
...
async def startup_event():
    """启动时加载模型和初始化资源"""
    global predictors, task_semaphore, thread_pool, TaskNames, settings
    
    # 加载Surya模块
    load_predictors, TaskNames_module, settings_module = load_surya_modules()
    TaskNames = TaskNames_module
    ...
    print(f"检测批处理大小: {getattr(settings, 'DETECTOR_BATCH_SIZE', 'auto')}")
    print(f"识别批处理大小: {getattr(settings, 'RECOGNITION_BATCH_SIZE', 'auto')}")
...
async def convert_pdf_to_markdown(
    ...
    """
    将PDF文件转换为Markdown格式（支持max_pages限制）
...
async def convert_pdf_from_url(
    ...
    """
    通过URL下载PDF文件并转换为Markdown格式（支持max_pages限制）
